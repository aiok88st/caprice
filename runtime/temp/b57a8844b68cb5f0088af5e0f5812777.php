<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:55:"F:\wamp\www\caprice/app/admin\view\articles\lists2.html";i:1506749087;s:51:"F:\wamp\www\caprice/app/admin\view\common\head.html";i:1505459050;s:51:"F:\wamp\www\caprice/app/admin\view\common\foot.html";i:1503623995;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="admin-main layui-anim layui-anim-upbit">
    ...
    <div class="demoTable">
        ...
        <a href="<?php echo url('lists2',['catid'=>$catid]); ?>" class="layui-btn">显示全部</a>
        <button type="button" class="layui-btn layui-btn-danger" id="delAll">批量删除</button>
        <a href="<?php echo url('enarticle/add',array('catid'=>$catid)); ?>" class="layui-btn" style="float:right;"><i class="fa fa-plus" aria-hidden="true"></i><?php echo lang('add'); ?>内容</a>
...
<script type="text/html" id="action">
    <a href="<?php echo url('enarticle/edit'); ?>?id={{d.id}}&catid={{d.catid}}" class="layui-btn layui-btn-mini">编辑</a>
...
<script>
    layui.use(['table'], function() {
        ...
        var tableIn = table.render({
            id: 'ad',
            elem: '#list',
            url: '<?php echo url("Articles/lists2",["catid"=>$catid]); ?>',
        ...
        table.on('tool(list)', function(obj) {
            ...
            if (obj.event === 'open') {
                ...
                $.post('<?php echo url("editState2"); ?>', {'id': data.id}, function (res) {
                    layer.close(loading);
                    if (res.status === 1) {
                        if (res.open === 1) {
                            obj.update({
                ...
                layer.confirm('您确定要删除吗？', function(index){
                    var loading = layer.load(1, {shade: [0.1, '#fff']});
                    $.post("<?php echo url('enarticle/listDel'); ?>",{'id':data.id},function(res){
                        layer.close(loading);
                        if(res.code===1){
                ...
                var loading = layer.load(1, {shade: [0.1, '#fff']});
                $.post('<?php echo url("enarticle/addHots"); ?>', {'id': data.id}, function (res) {
                    layer.close(loading);
                    if (res.code===1) {
                        if (res.status === 1) {
                            obj.update({
        ...
        $('body').on('blur','.list_order',function() {
            ...
            var catid = $("#catid").val();
            var loading = layer.load(1, {shade: [0.1, '#fff']});
            $.post('<?php echo url("enarticle/listorder"); ?>',{"id":id,"listorder":listorder,"catid":catid},function(res){
                layer.close(loading);
                if(res.code === 1){
                    layer.msg(res.msg, {time: 1000, icon: 1});
                    tableIn.reload();
        ...
        $('#delAll').click(function(){
            layer.confirm('确认要删除选中的内容吗？', {icon: 3}, function(index) {
                ...
                var loading = layer.load(1, {shade: [0.1, '#fff']});
                $.post("<?php echo url('enarticle/delAll'); ?>", {ids: ids}, function (data) {
                    layer.close(loading);
                    if (data.code === 1) {
